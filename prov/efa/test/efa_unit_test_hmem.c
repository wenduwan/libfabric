...
void test_efa_hmem_p2p_support(
	struct efa_resource **state, bool file_exists, char *prov_name,
	enum fi_hmem_iface iface, bool expected_p2p_support,
	enum efa_dmabuf_support_status expected_initial_dmabuf_support_status)
{
	int fd = -1, ret;
	...
	assert_int_equal(efa_domain->hmem_info[iface].dmabuf_support_status,
			 expected_initial_dmabuf_support_status);
}

void test_efa_dmabuf_support(struct efa_resource **state,
                             enum fi_hmem_iface iface,
                             enum efa_dmabuf_support_status initial_status,
                             bool require_dmabuf,
                             bool dmabuf_fd_supported,
                             bool dmabuf_mr_supported,
                             enum efa_dmabuf_support_status expected_final_status)
{
	bool iface_initialized;
	...
	hmem_ops[iface].get_dmabuf_fd = efa_mock_get_dmabuf_fd_return_mock;
	iface_initialized = hmem_ops[iface].initialized;
	hmem_ops[iface].initialized = true;
	...
	efa_domain->hmem_info[iface].dmabuf_support_status = initial_status;
	...
	if (!require_dmabuf && initial_status != EFA_DMABUF_NOT_SUPPORTED) {
		will_return(efa_mock_get_dmabuf_fd_return_mock,
			    dmabuf_fd_supported ? FI_SUCCESS : FI_EOPNOTSUPP);
	}

#if HAVE_EFA_DMABUF_MR
	g_efa_unit_test_mocks.ibv_reg_dmabuf_mr =
		efa_mock_ibv_reg_dmabuf_mr_set_error_and_return_mock;
	if (require_dmabuf || (initial_status != EFA_DMABUF_NOT_SUPPORTED &&
			       dmabuf_fd_supported)) {
		will_return(
			efa_mock_ibv_reg_dmabuf_mr_set_error_and_return_mock,
	...
	if (!require_dmabuf && (!dmabuf_fd_supported || !dmabuf_mr_supported)) {
	...
	assert_int_equal(efa_domain->hmem_info[iface].dmabuf_support_status, expected_final_status);
}
...
{
	test_efa_hmem_p2p_support(state, true, "NVIDIA peermem", FI_HMEM_CUDA,
				  true, EFA_DMABUF_UNINITIALIZED);
}
...
{
	test_efa_dmabuf_support(state, FI_HMEM_SYSTEM, EFA_DMABUF_NOT_SUPPORTED,
				false, false, false, EFA_DMABUF_NOT_SUPPORTED);
}

void test_efa_cuda_dmabuf_support_lazy_init_success(struct efa_resource **state)
{
	test_efa_dmabuf_support(state, FI_HMEM_CUDA, EFA_DMABUF_UNINITIALIZED,
				false, true, true, EFA_DMABUF_SUPPORTED);
}

void test_efa_cuda_dmabuf_support_lazy_init_get_fd_fail_fallback(struct efa_resource **state)
{
	test_efa_dmabuf_support(state, FI_HMEM_CUDA, EFA_DMABUF_UNINITIALIZED,
				false, false, false, EFA_DMABUF_NOT_SUPPORTED);
}

void test_efa_cuda_dmabuf_support_lazy_init_mr_fail_fallback(struct efa_resource **state)
{
	test_efa_dmabuf_support(state, FI_HMEM_CUDA, EFA_DMABUF_UNINITIALIZED,
				false, true, false, EFA_DMABUF_NOT_SUPPORTED);
}

void test_efa_cuda_dmabuf_support_always_ibv_reg_mr(struct efa_resource **state)
{
	test_efa_dmabuf_support(state, FI_HMEM_CUDA, EFA_DMABUF_NOT_SUPPORTED,
				false, false, false, EFA_DMABUF_NOT_SUPPORTED);
}

void test_efa_cuda_dmabuf_support_mr_fail_unexpectedly(struct efa_resource **state)
{
	test_efa_dmabuf_support(state, FI_HMEM_CUDA, EFA_DMABUF_SUPPORTED,
				true, false, false, EFA_DMABUF_SUPPORTED);
}

void test_efa_cuda_dmabuf_support_require_dmabuf_fail_no_fallback(struct efa_resource **state)
{
	test_efa_dmabuf_support(state, FI_HMEM_CUDA, EFA_DMABUF_NOT_SUPPORTED,
				true, false, false, EFA_DMABUF_NOT_SUPPORTED);
}

void test_efa_neuron_dmabuf_support_lazy_init_success(struct efa_resource **state)
{
	test_efa_dmabuf_support(state, FI_HMEM_NEURON, EFA_DMABUF_UNINITIALIZED,
				false, true, true, EFA_DMABUF_SUPPORTED);
}

void test_efa_neuron_dmabuf_support_lazy_init_get_fd_fail_fallback(struct efa_resource **state)
{
	test_efa_dmabuf_support(state, FI_HMEM_NEURON, EFA_DMABUF_UNINITIALIZED,
				false, false, false, EFA_DMABUF_NOT_SUPPORTED);
}

void test_efa_neuron_dmabuf_support_lazy_init_mr_fail_fallback(struct efa_resource **state)
{
	test_efa_dmabuf_support(state, FI_HMEM_NEURON, EFA_DMABUF_UNINITIALIZED,
				false, true, false, EFA_DMABUF_NOT_SUPPORTED);
}

void test_efa_neuron_dmabuf_support_always_ibv_reg_mr(struct efa_resource **state)
{
	test_efa_dmabuf_support(state, FI_HMEM_NEURON, EFA_DMABUF_NOT_SUPPORTED,
				false, false, false, EFA_DMABUF_NOT_SUPPORTED);
}

void test_efa_neuron_dmabuf_support_mr_fail_unexpectedly(struct efa_resource **state)
{
	test_efa_dmabuf_support(state, FI_HMEM_NEURON, EFA_DMABUF_SUPPORTED,
				true, false, false, EFA_DMABUF_SUPPORTED);
}
...
/* Synapseai requires dmabuf so skip lazy initialization tests */
void test_efa_synapseai_dmabuf_support_mr_fail_unexpectedly(struct efa_resource **state)
{
	test_efa_dmabuf_support(state, FI_HMEM_SYNAPSEAI, EFA_DMABUF_SUPPORTED,
				true, false, false, EFA_DMABUF_SUPPORTED);
}

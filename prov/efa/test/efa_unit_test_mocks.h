...
int efa_mock_get_dmabuf_fd_return_mock(const void *addr, uint64_t size, int *fd, uint64_t *offset);

#if HAVE_EFA_DMABUF_MR
struct ibv_mr *__real_ibv_reg_dmabuf_mr(struct ibv_pd *pd, uint64_t offset, size_t length, uint64_t iova, int fd, int access);
struct ibv_mr *efa_mock_ibv_reg_dmabuf_mr_set_error_and_return_mock(struct ibv_pd *pd, uint64_t offset, size_t length, uint64_t iova, int fd, int access);
#endif
